...
import { getDueCount, getProgressByBook, getStreakCount } from "@/lib/actions";
...
export default async function BookDashboard({ params }: { params: { slug: string } }) {
  const { slug } = params;
    ...
    <div className="mx-auto max-w-5xl space-y-8">
      <div className="animate-fade-in-up">
        ...
        <p className="mt-1 text-fg/60">
          Đang học <span className="font-semibold text-fg">{book.nameKr}</span> — hôm nay học gì nào?
        </p>
      </div>

      <div className="stagger-children grid grid-cols-1 gap-4 sm:grid-cols-3">
        {stats.map((s) => (
          <div key={s.label} className={`card-cute bg-gradient-to-br ${s.bg} p-5`}>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Link href="/hangeul" className="card-cute group flex items-center gap-3 p-5 hover:border-secondary/40">
        ...
        <Link href={`/${slug}/vocabulary`} className="card-cute group flex items-center gap-3 p-5 hover:border-secondary/40">
          <div className="rounded-2xl bg-secondary/10 p-3 text-secondary transition-transform group-hover:scale-110">
            <Languages size={22} />
          </div>
      ...
      <div>
        <div className="mb-4 flex items-center gap-2">
          ...
          <h3 className="font-display text-lg font-bold">{book.units} bài học</h3>
        </div>
