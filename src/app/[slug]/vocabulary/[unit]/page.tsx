...
export default async function BookVocabUnitPage({
  params,
}: {
  ...
  const raw = await getVocabByUnit(num, slug);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <Link href={`/${slug}/vocabulary`} className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-primary">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Link href={`/${slug}/grammar/${num}`} className="rounded-lg bg-secondary/10 px-4 py-2 text-sm font-semibold text-secondary">
            Ngữ pháp →
          </Link>
        </div>
      </div>
